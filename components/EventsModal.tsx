...
const EventsModal: React.FC<EventsModalProps> = ({ isOpen, onClose, events }) => {
  ...
  return (
                    ...
                    <ul className="divide-y divide-black/5">
                        {events.map((event, index) => (
                            <li 
                              ...
                              className="group bg-white/50 hover:bg-white transition-colors p-6 md:p-8 flex flex-col md:flex-row gap-6 md:gap-8 items-start cursor-pointer"
                            >
                                ...
                                <div className="shrink-0 relative w-full md:w-48 aspect-video md:aspect-[4/3] rounded-xl overflow-hidden bg-zinc-200 border border-black/5">
                                    <img 
                                        src={event.image || `https://images.unsplash.com/photo-1547891654-e66ed7ebb968?q=80&w=400&auto=format&fit=crop`} 
                                        alt={event.imageAlt || "Event Placeholder"} 
                                ...
                                {/* Content Column */}
                                <div className="grow py-1 flex flex-col h-full">
                                    <h3 className="text-xl md:text-2xl font-bold mb-3 group-hover:text-[#105CB3] transition-colors">
                                        {event.title}
                                    </h3>
                                    <p className="text-zinc-500 leading-relaxed max-w-xl mb-4">
                                        {event.description}
                                    </p>
                                    <div className="mt-auto">
                                      <button className="text-sm font-bold uppercase tracking-wider flex items-center gap-2 text-[#105CB3] group-hover:translate-x-2 transition-transform">
    ...
    {/* Event Detail Modal (rendered outside the main modal's AnimatePresence to layer on top) */}
    <EventDetailModal 
      isOpen={!!selectedEvent} 
